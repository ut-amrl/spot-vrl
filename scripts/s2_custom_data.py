...
class s2CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # net.eval()
        with torch.no_grad():
            visual_encoding, inertial_encoding, ls1, ls2 = net(torch.from_numpy(np.expand_dims(np.array([main_patch_1]),1)).float(),torch.from_numpy(np.moveaxis(np.expand_dims(inertial_data,1),-1,0)).float(),s1=False)
        ...
        cluster = self.model.predict(scaled_features)
...
class s2DataLoader(pl.LightningDataModule):
	...
	def val_dataloader(self):
		return DataLoader(self.val_dataset, batch_size=self.batch_size, shuffle=False, num_workers=0, drop_last=True if len(self.val_dataset) % self.batch_size != 0 else False)
