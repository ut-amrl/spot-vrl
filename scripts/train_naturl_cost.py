...
class NATURLCostModel(pl.LightningModule):
    def __init__(self, latent_size=128, visual_encoder_weights=None):
    ...
    def compute_preference_loss(self, cost, preference_labels):
        ...
        for i in range(cost.shape[0]):
            ...
            if preference_labels[i] < preference_labels[j]:
                loss += self.softmax_with_temp(cost[i], cost[j])
            elif preference_labels[i] > preference_labels[j]:
                loss += self.softmax_with_temp(cost[j], cost[i])
            else:
